...
def get_pie_chart(site_dropdown):
    ...
    filtered_df = filtered_df.filter(['Launch Site', 'class']) 
    if site_dropdown == 'ALL':
        filtered_df = filtered_df[filtered_df['class'] == 1]
        filtered_df = filtered_df.groupby(['Launch Site'], as_index=False).count() 
        fig = px.pie(filtered_df, values='class', hole=.3, 
        names='Launch Site', 
        ...
        fig = px.pie(filtered_df, values='Launch Site', hole=.3,
        names='class',
        title='Successful vs Unsuccessful Launches for ' + site_dropdown, 
        color_discrete_sequence=px.colors.sequential.Teal_r) 
        return fig
...
def get_scatter_chart(site_dropdown, payload_slider): 
    ...
    sliderange = (filtered_df['Payload Mass (kg)'] >= low) & (filtered_df['Payload Mass (kg)'] <= high)
    if site_dropdown == 'ALL': 
        fig = px.scatter(filtered_df[sliderange], x='Payload Mass (kg)', y='class', 
